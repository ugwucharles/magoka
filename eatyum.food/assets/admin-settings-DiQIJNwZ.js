...
function Ur() {
  ...
  const Ke = () => {
  ...
  }, { data: v } = _({ queryKey: ["/api/magoka/orderpro/catalog/sync/jobs", S], enabled: !!S, queryFn: () => __async(null, null, function* () {
    if (!S) return null;
    const s = yield fetch(`/api/magoka/orderpro/catalog/sync/jobs/${S}`, { method: "GET", headers: Ye(), credentials: "include", cache: "no-store" });
    if (!s.ok) {
    ...
    const i = yield fetch("/api/magoka/orderpro/catalog/sync/jobs?scope=all&mode=bulk", { method: "POST", headers: a, credentials: "include" });
    if (!i.ok) {
    ...
    J(s.jobId), I.setQueryData(["/api/magoka/orderpro/catalog/sync/jobs", s.jobId], s);
  }, onError: (s) => {
  ...
  x.useEffect(() => {
    const s = ["Popular Items Display", "Homepage Layout", "Referral Reward", "Rider Payout Cap", "Paystack Payout Webhook", "Chowdeck Integration", "Sales Channels", "OrderPro Catalog Sync", "Data Management"], a = () => {
      document.querySelectorAll(".space-y-6 > div").forEach((i) => {
        const u = i.textContent || "";
        s.some((f) => u.includes(f)) && (i.style.display = "none");
      });
    };
    a();
    const l = window.setTimeout(a, 250);
