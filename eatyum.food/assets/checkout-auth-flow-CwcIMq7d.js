...
function Fe({ open: O, onClose: M, onAuthenticated: W, onContinueAsGuest: K }) {
  ...
  const R = () => __async(null, null, function* () {
    ...
    try {
      ...
      o.ok ? t.token && t.user ? X(t.token, t.user, "Login successful!") : (L({ email: s ? l : void 0, phone: s ? void 0 : l }), D(s ? "email" : "phone"), h("otp-verification"), E(60), n({ title: "Verification Code Sent", description: `Check ${s ? "your email" : "your phone"} for the verification code`, variant: "default" })) : n({ title: "Error", description: t.message || "Failed to send verification code", variant: "destructive" });
    } catch (s) {
    ...
    try {
      ...
      if (o.ok) t.token && t.user ? X(t.token, t.user, "Account created successfully!") : (L({ email: s, phone: l, tempUserId: "signup" }), D("email"), h("otp-verification"), E(60), n({ title: "Registration Initiated", description: t.message || "Please verify your account with the OTP sent to you." }));
